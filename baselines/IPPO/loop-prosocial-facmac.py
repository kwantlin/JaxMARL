...
def make_train(config):
    ...
    def train(rng):
        ...
        options = orbax.checkpoint.CheckpointManagerOptions(max_to_keep=1, best_fn=lambda metric: metric["episode_returns"][:,0].mean(), best_mode='max')
        mngr = orbax.checkpoint.CheckpointManager(
            'ckpt_best_state', orbax.checkpoint.AsyncCheckpointer(orbax.checkpoint.StandardCheckpointHandler()),
            options)
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                train_state, env_state, last_obs, rng, step = runner_state
                ...
                runner_state = (train_state, env_state, obsv, rng, step)
            ...
            print("returned metrics", metric)
            # state_save_args = jax.tree.map(lambda _: orbax.checkpoint.SaveArgs(), state)
            mngr.save(step, args=ocp.args.StandardSave(train_state), metrics=metric)
            runner_state = (train_state, env_state, last_obs, rng, step+1)
            return runner_state, metric

        rng, _rng = jax.random.split(rng)
        step = 0
        runner_state = (train_state, env_state, obsv, _rng, step)
...
def main(config):
    ...
    if config['SAVE_PATH'] is not None:
        ...
        orbax_checkpointer.save('/n/fs/klips/JaxMARL/baselines/IPPO/tmp/prosocial', ckpt, save_args=save_args)
